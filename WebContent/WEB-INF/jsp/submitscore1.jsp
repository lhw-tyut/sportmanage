...
<h1 align="center">${msg1}</h1>
 ...
 <th>项目</th><th>成绩</th></tr></thead>
						<tbody>
						<form action="${pageContext.request.contextPath }/${msg2}" method="post">
...
<td>${row.s_sex }</td>
<td>${row.s_type }</td>
<td><input type="text" name=<%=sporter_count %> ></td>
<input type="hidden" name="<%=sporter_count+20 %>" value="${row.s_id}" />
</tr>
<%sporter_count++; %>
							</c:forEach>
							<input type="hidden" name="sum" value="<%=sporter_count %>" />
						</tbody>
					</table>
					...
					<div class="col-md-12 text-right">
						...
						<itheima:page url="	${pageContext.request.contextPath }/${msg3 }"/>
					</div>
					<!-- /.panel-body -->
				</div>
				<!-- /.panel -->
